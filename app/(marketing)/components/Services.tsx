          ...
          {services.map((service) => {
            ...
            const cardContent = (
              <>
                ...
                {href ? (
                  <p className="mt-6 text-xs font-semibold text-brand-teal dark:text-accent-cyan">
                    Ver más →
                  </p>
                ) : null}
              </>
            ...
            return href ? (
              <Link
                key={service.title}
                href={href}
                className="card transition-shadow hover:shadow-lg hover:-translate-y-0.5 duration-200"
              >
                {cardContent}
              </Link>
            ) : (
