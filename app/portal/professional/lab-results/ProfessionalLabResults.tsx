...
export function ProfessionalLabResults() {
  ...
  const loadAttachments = async () => {
    const response = await fetchWithRetry(`/api/professional/attachments?kind=${AttachmentKind.LAB}`);
  ...
  const handleUpload = async (file?: File | null, url?: string | null) => {
    ...
    let filename = file?.name ?? url?.split("/").pop() ?? "Lab Result";
  ...
  return (
    <div className="space-y-6">
      <header>
        <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Lab Results</p>
      ...
      <div className="rounded-3xl border border-slate-200 bg-white/80 p-6 shadow-xs dark:border-slate-800 dark:bg-slate-900/60">
        ...
        <div className="mt-4 space-y-3">
          {attachments.length === 0 ? (
            <p className="text-sm text-slate-500">No lab results uploaded yet.</p>
          ) : (
            attachments
