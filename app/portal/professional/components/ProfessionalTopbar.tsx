...
export function ProfessionalTopbar({ userName, onMenuClick }: ProfessionalTopbarProps) {
  ...
  return (
    <header className="sticky top-0 z-30 flex flex-wrap items-center justify-between gap-4 border-b border-slate-200 bg-white/80 px-6 py-4 backdrop-blur-sm dark:border-surface-muted/60 dark:bg-surface-base/80">
      <div className="flex items-center gap-3">
        <button
          type="button"
          className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-slate-200 text-slate-600 transition hover:text-slate-900 focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-brand-teal/60 dark:border-surface-muted/60 dark:text-slate-200 md:hidden"
          onClick={onMenuClick}
      ...
      <div className="relative flex flex-1 items-center justify-end gap-3">
        <div className="relative hidden w-full max-w-sm items-center md:flex">
          ...
          <input
            ...
            className="w-full rounded-full border border-slate-200 bg-white py-2.5 pl-10 pr-4 text-sm text-slate-600 shadow-xs shadow-slate-100/60 outline-hidden transition focus-visible:ring-2 focus-visible:ring-brand-teal/60 dark:border-surface-muted/60 dark:bg-surface-base dark:text-slate-100"
          />
          {query ? (
            <div className="absolute left-0 right-0 top-12 z-20 rounded-2xl border border-slate-200 bg-white p-2 shadow-xl dark:border-surface-muted/60 dark:bg-surface-base">
              {isSearching ? (
                  ...
                  {results.map((result) => (
                    <li key={result.id}>
                      <button
                        ...
                        className="flex w-full flex-col gap-1 rounded-xl px-3 py-2 text-left text-sm text-slate-600 transition hover:bg-slate-100 dark:text-slate-200 dark:hover:bg-surface-muted"
                      >
        ...
        <div className="relative">
          <button
            type="button"
            className="relative inline-flex h-11 w-11 items-center justify-center rounded-full border border-slate-200 text-slate-600 transition hover:text-slate-900 focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-brand-teal/60 dark:border-surface-muted/60 dark:text-slate-200"
          ...
          {isNotificationsOpen ? (
            <div className="absolute right-0 mt-3 w-80 rounded-2xl border border-slate-200 bg-white p-3 text-sm shadow-xl dark:border-surface-muted/60 dark:bg-surface-base">
              ...
              {notifications.length === 0 ? (
                ...
                <ul className="space-y-2">
                  {notifications.map((notification) => (
                    <li key={notification.id} className="rounded-xl border border-slate-200 px-3 py-2 dark:border-surface-muted/60">
