  ...
  return (
    <div className="space-y-4">
      {appointments.length === 0 ? (
        <p className="rounded-xl border border-dashed border-slate-200 p-6 text-sm text-slate-500">No hay citas registradas.</p>
      ) : (
        appointments.map((appointment) => (
          <div key={appointment.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
              <div>
                <p className="text-xs font-semibold uppercase tracking-wide text-brand-teal">{appointment.status}</p>
                <h3 className="text-lg font-semibold text-slate-900">{appointment.reason}</h3>
                <p className="text-sm text-slate-600">{formatDateRange(appointment.timeSlot.startAt, appointment.timeSlot.endAt)}</p>
            ...
            {appointment.notes ? <p className="mt-3 text-sm text-slate-600">Notas: {appointment.notes}</p> : null}
          </div>
        ))
      )}
    </div>
