...
export async function PATCH(request: Request) {
  ...
  const items = await prisma.homepageContactSupportItem.findMany({ select: { id: true } });

  if (items.length !== body.orderedIds.length) {
    return errorResponse("La lista de orden no coincide con la cantidad de ítems de soporte.", 400);
  }

  if (new Set(body.orderedIds).size !== body.orderedIds.length) {
    return errorResponse("La lista de orden contiene ítems de soporte duplicados.", 400);
  }

  const expected = new Set(items.map((item: { id: string }) => item.id));
  const received = new Set(body.orderedIds);
  if (received.size !== expected.size || [...received].some((id) => !expected.has(id))) {
    return errorResponse("La lista de orden contiene ítems de soporte inválidos.", 400);
  }
  ...
  await logAuditEvent({
    ...
    action: "homepage.contact-support-items.reordered",
