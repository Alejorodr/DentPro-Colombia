...
export async function GET(request: Request) {
  ...
  try {
    ...
    metric.end({ status: "ok", itemCount: items.events.length, extra: { actor: sessionResult.user.role, requestId } });
    trace.end({ status: "ok", result: "ok", itemCount: items.events.length });
    ...
    return errorResponse("No se pudo cargar el feed de actividad.", 500);
  }
}
