  ...
  patient: [
    {
      id: "agenda",
      title: "Mi agenda",
      description: "Consulta tus próximas citas y recibe recordatorios automáticos.",
      highlights: [
        "Confirma o reprograma directamente desde el tablero",
        "Añade notas previas para tu especialista",
      ],
      routes: [
        { label: "Próximas citas", path: "/patient/agenda" },
        { label: "Historial clínico", path: "/patient/historial" },
      ],
      icon: "agenda",
    },
    {
      id: "pacientes",
      title: "Mis datos",
      description: "Mantén tu información de contacto y consentimientos actualizados.",
      highlights: [
      ...
      routes: [
        ...
        { label: "Documentos", path: "/patient/documentos" },
      ],
      icon: "patients",
    },
  ],
  professional: [
    {
      ...
      description: "Visualiza tus citas confirmadas y pendientes por paciente.",
      highlights: [
        "Bloquea horarios y marca asistencias",
        "Accede al detalle clínico antes de la cita",
      ],
      routes: [
        { label: "Agenda", path: "/professional/agenda" },
        { label: "Pendientes", path: "/professional/tareas" },
      ],
      icon: "agenda",
    },
    {
      ...
      description: "Revisa historial, órdenes y documentos compartidos contigo.",
      highlights: [
        "Filtra por urgencia o especialidad",
        "Comparte indicaciones postoperatorias",
      ],
      routes: [
        { label: "Pacientes", path: "/professional/pacientes" },
        { label: "Evoluciones", path: "/professional/evoluciones" },
      ],
      icon: "patients",
    },
  ],
  reception: [
    {
      ...
      description: "Administra citas, confirma asistencia y reasigna especialistas.",
      highlights: [
      ...
      routes: [
        ...
        { label: "Recepción", path: "/reception/mostrador" },
      ],
      icon: "agenda",
    },
    {
      id: "pacientes",
      title: "Pacientes",
      description: "Consulta datos de contacto, pólizas y saldos pendientes.",
      highlights: [
      ...
      routes: [
        ...
        { label: "Pagos", path: "/reception/pagos" },
      ],
      icon: "patients",
    },
  ],
  admin: [
    {
      id: "insights",
      title: "Indicadores",
      description: "Sigue la ocupación, cancelaciones y desempeño por especialidad.",
      highlights: [
    ...
    },
    {
      ...
      description: "Gestiona accesos, roles y disponibilidad de profesionales.",
      highlights: [
...
function getSectionIcon(section: DashboardSection) {
  switch (section.icon) {
    case "agenda":
      return <CalendarCheck className="h-6 w-6 text-brand-teal" weight="bold" aria-hidden="true" />;
    case "patients":
      return <UsersThree className="h-6 w-6 text-brand-indigo" weight="bold" aria-hidden="true" />;
    case "tasks":
      return <ClipboardText className="h-6 w-6 text-brand-sky" weight="bold" aria-hidden="true" />;
    case "insights":
      return <ChartLineUp className="h-6 w-6 text-amber-500" weight="bold" aria-hidden="true" />;
    default:
      return null;
  }
}
...
export default async function RoleDashboardPage({
  ...
  const sections = sectionsByRole[requestedRole];
  const userLabel = roleLabels[requestedRole];
  ...
  return (
    <main className="space-y-8 py-6">
      <header className="rounded-2xl bg-white px-6 py-5 shadow-sm ring-1 ring-slate-200 transition-colors duration-300 dark:bg-surface-elevated dark:ring-surface-muted">
        <p className="text-xs font-semibold uppercase tracking-wide text-brand-teal dark:text-accent-cyan">{userLabel}</p>
        <div className="mt-2 flex flex-wrap items-center gap-3">
          <h1 className="text-3xl font-semibold text-slate-900 dark:text-white">Tablero de {userLabel}</h1>
        ...
        <p className="mt-3 max-w-3xl text-sm text-slate-600 dark:text-slate-300">
          Administra tus módulos principales desde aquí. Usa las rutas sugeridas para navegar por agenda, pacientes y reportes según tu rol.
        </p>
        <div className="mt-4 flex items-center gap-3 text-xs text-slate-600 dark:text-slate-300">
          <Lifebuoy className="h-4 w-4" weight="bold" aria-hidden="true" />
          <span>Si necesitas cambiar de rol, cierra sesión e ingresa con tus credenciales asignadas.</span>
        </div>
      </header>
        ...
        {sections.map((section) => (
          ...
          >
            ...
            <ul className="space-y-2 text-sm text-slate-700 dark:text-slate-200">
              {section.highlights.map((item) => (
                <li key={item} className="flex items-start gap-2">
                  <span className="mt-1 h-2 w-2 rounded-full bg-brand-teal" aria-hidden="true" />
                  <span>{item}</span>
                </li>
              ))}
            </ul>
            ...
            <div className="rounded-xl border border-slate-200 bg-slate-50/70 p-4 text-sm text-slate-700 transition-colors duration-300 dark:border-surface-muted dark:bg-surface-muted/40 dark:text-slate-200">
              <p className="mb-2 font-semibold text-slate-900 dark:text-white">Rutas sugeridas</p>
