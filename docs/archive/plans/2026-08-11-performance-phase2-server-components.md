...
- Per the parent design spec (`docs/archive/specs/2026-08-11-performance-core-web-vitals-design.md`), the portal is audited, not blanket-converted: "portal dashboards are inherently interactive... real candidates are expected to be few or none. Report findings before converting anything there." Only portal files individually verified to have zero interactivity signals are converted in this plan; everything else stays client and is documented as correctly staying client.
...
- Manual browser verification (2026-08-11, follow-up pass): ran `pnpm run dev`, logged in as an existing `ADMINISTRADOR` user, and checked both pages in Chrome.
  - Homepage `/`: info bar renders (location + map-pin icon, schedule + clock icon, WhatsApp link, email link, social icons). Clicked the WhatsApp link (opened `api.whatsapp.com/send/?phone=573237968435...` in a new tab, `target="_blank"` intact) and the Instagram icon (opened `instagram.com/dentprocol` in a new tab). No console errors, no hydration-mismatch warnings on this route.
  - `/portal/admin`: all 4 converted components render with real data — 4 KPI stat cards with `TrendUp` icons, revenue trend sparkline chart, "Equipo activo" staff list with avatar fallback and status dot, "Agenda del día" appointments table with status badges. "Ver todos" (staff) and "Ver todas" (appointments) links navigate correctly to `/portal/admin/staff` and `/portal/admin/appointments`. No hydration-mismatch warnings or RSC boundary errors on this route (the only console messages were a GrowthBook browser-extension log, unrelated to the app).
  - Note: a hydration-mismatch warning was observed once on `/auth/login` (a Next.js dev-mode metadata-`Suspense`-boundary `hidden` attribute mismatch, unrelated to any file this plan touches — that route doesn't import InfoBar or the 4 admin components). Not in scope to fix here; flagged for awareness only.
  - Both checks confirm no visual or behavioral regression from the Task 1-3 conversions. Plan is now fully closed.
