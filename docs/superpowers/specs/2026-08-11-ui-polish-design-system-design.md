...
1. **Icon barrel violation**: exactly one file imports Phosphor icons directly instead of through the mandated barrel — `app/(dashboard)/components/SpecialistsShowcase.tsx` imports `CaretLeft`/`CaretRight` from `@phosphor-icons/react` instead of `@/components/ui/Icon`.
