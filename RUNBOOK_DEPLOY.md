...
- `OPS_ENABLED`, `OPS_KEY`, `OPS_IP_ALLOWLIST`, `SEED_ADMIN_EMAIL`, `SEED_ADMIN_PASSWORD`
...
- En producción `OPS_ENABLED` debe permanecer en `false` por defecto. Si se habilita, configure `OPS_KEY` y `OPS_IP_ALLOWLIST` (IPs separadas por coma) para restringir el acceso.
